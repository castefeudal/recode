...
class_name AndroidHealthConnectProvider

const SINGLETON := "RecodeHealthConnect"
...
func request_permissions(categories: Array[String]) -> Dictionary:
    if not is_available():
        return {"granted": [], "denied": categories, "reason": "health_connect_bridge_unavailable"}
    var bridge = Engine.get_singleton(SINGLETON)
    if not bridge.has_method("request_permissions"):
        return {"granted": [], "denied": categories, "reason": "health_connect_method_unavailable"}
