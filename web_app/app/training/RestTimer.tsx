...
export function RestTimer({ seconds, lang }: { seconds: number; lang: "ru" | "en" }) {
  ...
  return <section className={`restTimer ${complete ? "done" : ""}`} aria-label={lang === "ru" ? "Таймер отдыха" : "Rest timer"}>
    <div><small>REST</small><b>{String(minutes).padStart(2, "0")}:{String(secs).padStart(2, "0")}</b></div>
    <div>
      <button disabled={complete} onClick={() => setRunning((value) => !value)}>{complete ? (lang === "ru" ? "Готово" : "Done") : running ? (lang === "ru" ? "Пауза" : "Pause") : (lang === "ru" ? "Старт" : "Start")}</button>
